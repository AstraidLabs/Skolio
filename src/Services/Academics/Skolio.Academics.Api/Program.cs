...
app.MapGet("/", (Microsoft.Extensions.Options.IOptions<AcademicsServiceOptions> options) => Results.Ok(new { service = options.Value.ServiceName, status = "phase-4-ready", publicBaseUrl = options.Value.PublicBaseUrl }));
app.Run();
