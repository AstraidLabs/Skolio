...
builder.Services.AddHealthChecks();builder.Services.AddOpenApi();
...
app.MapGet("/", (Microsoft.Extensions.Options.IOptions<AcademicsServiceOptions> options) => Results.Ok(new { service = options.Value.ServiceName, status = "phase-5-auth-ready", publicBaseUrl = options.Value.PublicBaseUrl }));
app.Run();
