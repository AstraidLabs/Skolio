...
export function createCommunicationApi(http: ReturnType<typeof createHttpClient>) {
  return {
    ...
    sendMessage: (payload: Omit<Message, 'id' | 'sentAtUtc'> & { sentAtUtc: string }) => http<Message>('communication', '/api/communication/conversations/messages', { method: 'POST', body: JSON.stringify(payload) }),
    notifications: async (recipientUserId: string) => {
      const result = await http<PagedResult<Notification>>('communication', `/api/communication/notifications?recipientUserId=${recipientUserId}`);
      return result.items;
    }
  };
}
