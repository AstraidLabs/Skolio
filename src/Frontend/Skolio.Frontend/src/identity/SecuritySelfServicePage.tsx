...
export function SecuritySelfServicePage({ api }: { api: ReturnType<typeof createIdentityApi> }) {
  ...
  return (
    <section className="space-y-4">
      <SectionHeader title={t('securityTitle')} description={t('securityDescription')} action={<button className="sk-btn sk-btn-secondary" onClick={load} type="button">{t('reloadLabel')}</button>} />
      ...
      <Card>
        <p className="font-semibold text-sm">{t('securitySummaryTitle')}</p>
        <div className="mt-2 grid gap-2 md:grid-cols-2">
          <p className="text-sm">{t('securityCurrentEmail')}: {summary.currentEmail}</p>
      ...
      <Card>
        <p className="font-semibold text-sm">{t('securityChangePasswordTitle')}</p>
        <div className="mt-2 grid gap-2 md:grid-cols-3">
          <input className="sk-input" type="password" placeholder={t('securityCurrentPassword')} value={changePasswordDraft.currentPassword} onChange={(e) => setChangePasswordDraft((v) => ({ ...v, currentPassword: e.target.value }))} />
          <input className="sk-input" type="password" placeholder={t('securityNewPassword')} value={changePasswordDraft.newPassword} onChange={(e) => setChangePasswordDraft((v) => ({ ...v, newPassword: e.target.value }))} />
          <input className="sk-input" type="password" placeholder={t('securityConfirmNewPassword')} value={changePasswordDraft.confirmNewPassword} onChange={(e) => setChangePasswordDraft((v) => ({ ...v, confirmNewPassword: e.target.value }))} />
        </div>
        <div className="mt-2">
      ...
      <Card>
        <p className="font-semibold text-sm">{t('securityChangeEmailTitle')}</p>
        <div className="mt-2 grid gap-2 md:grid-cols-2">
          <input className="sk-input" type="password" placeholder={t('securityCurrentPasswordReauth')} value={changeEmailDraft.currentPassword} onChange={(e) => setChangeEmailDraft((v) => ({ ...v, currentPassword: e.target.value }))} />
          <input className="sk-input" type="email" placeholder={t('securityNewEmail')} value={changeEmailDraft.newEmail} onChange={(e) => setChangeEmailDraft((v) => ({ ...v, newEmail: e.target.value }))} />
        </div>
        <div className="mt-2">
      ...
      <Card>
        <p className="font-semibold text-sm">{t('securityMfaManagementTitle')}</p>
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          <button className="sk-btn sk-btn-secondary" type="button" onClick={startMfaSetup}>{t('securityStartMfaSetupAction')}</button>
        </div>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-3">
          <input className="sk-input" type="password" placeholder={t('securityCurrentPasswordReauth')} value={mfaDisableDraft.currentPassword} onChange={(e) => setMfaDisableDraft((v) => ({ ...v, currentPassword: e.target.value }))} />
          ...
          <button className="sk-btn sk-btn-secondary" type="button" onClick={disableMfa}>{t('securityDisableMfaAction')}</button>
        </div>

        <div className="mt-3 grid gap-2 md:grid-cols-2">
          <input className="sk-input" type="password" placeholder={t('securityCurrentPasswordReauth')} value={mfaRegeneratePassword} onChange={(e) => setMfaRegeneratePassword(e.target.value)} />
          <button className="sk-btn sk-btn-secondary" type="button" onClick={regenerateCodes}>{t('securityRegenerateRecoveryCodesAction')}</button>
        </div>
      </Card>

      <Card>
        <p className="font-semibold text-sm">{t('securityRecoveryCodesTitle')}</p>
